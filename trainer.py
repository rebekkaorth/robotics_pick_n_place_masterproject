    ...
    def backprop(self, color_heightmap, depth_heightmap, primitive_action, best_pix_ind, label_value):
        ...
        if self.method == 'reactive':
            ...
            if primitive_action == 'push':
                ...
                push_predictions, grasp_predictions, state_feat = self.forward(color_heightmap, depth_heightmap,
                                                                               is_volatile=False,
                                                                               specific_rotation=best_pix_ind[0])

                if self.use_cuda:
                    loss = self.push_criterion(self.model.output_prob[0][0],
                ...
                if self.use_cuda:
                    loss = self.grasp_criterion(self.model.output_prob[0][1],
                                                Variable(torch.from_numpy(label).long().cuda()))
                else:
